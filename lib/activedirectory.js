...
function search(baseDN, opts, callback) {
  ...
  if (!_.any(controls, function(control) { return(control instanceof ldap.PagedResultsControl); })) {
    ...
    controls.push(ActiveDirectory.prototype._getLdapPagedResultsControl(defaultPageSize)));
  }
  ...
  function searchDeletedObjects(baseDN, opts) {
    if (!_.any(opts.controls, function(control) { return(control.type === '1.2.840.113556.1.4.417'); })) {
      log.debug('Adding ShowDeletedOidControl(1.2.840.113556.1.4.417) to search (%s) with filter "%s" for %j',
                baseDN, truncateLogOutput(opts.filter), _.any(opts.attributes) ? opts.attributes : '[*]');
      opts.controls.push(ActiveDirectory.prototype._getLdapShowDeletedOidControl());
    }
...
/**
 * Retrieves an instance of the specified ldap control / extension the underlying ldapjs library.
 *
 * @param {Object} properties the attributes for the control. { type: '1.2.840.113556.1.4.417', criticality: true }
 * @returns {Object}
 */
ActiveDirectory.prototype._getLdapControl = function getLdapControl(properties) {
  return(new ldap.Control(properties));
};

/**
 * Retrieves an instance of the PagedResultsControl object from the underlying ldapjs library.
 *
 * @param {Number} pageSize the optional default page size to return. If not specified, defaults to 1000.
 * @returns {Object}
 */
ActiveDirectory.prototype._getLdapPagedResultsControl = function getPagedResultsControl(pageSize) {
  return(new ldap.PagedResultsControl({ value: { size: defaultPageSize || defaultPageSize } }));
};

/**
 * Retrieves an instance of an LDAP control extension for retrieving deleted items from the underlying ldapjs library.
 *
 * @returns {Object}
 */
ActiveDirectory.prototype._getLdapShowDeletedOidControl = function getShowDeletedOidControl() {
  return(ActiveDirectory.prototype._getLdapControl({ type: '1.2.840.113556.1.4.417', criticality: true }));
};
